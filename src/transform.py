...
def _drop_internal_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Hint: use a list comprehension on df.columns
    #   internal_cols = [col for col in df.columns if col.startswith('_')]
    #   df = df.drop(columns=internal_cols)
    #   print(f"    🧹 {len(internal_cols)} internal columns dropped")
    #   return df
...
def transform_products() -> pd.DataFrame:
    ...
    # TODO: Step 2 — Normalize the 'tags' column (replace '|' with ',')
    # Hint: df["tags"] = df["tags"].str.replace("|", ", ")

    # TODO: Step 3 — Validate price_usd (must be > 0)
    # Hint: df = df[df["price_usd"] > 0]

    # TODO: Step 4 — Convert booleans
    # Hint: df["is_active"] = df["is_active"].astype(bool)

    # TODO: Step 5 — Load into Silver
...
def transform_users() -> pd.DataFrame:
    ...
    # Hint: df["loyalty_tier"] = df["loyalty_tier"].fillna("none")

    # TODO: Step 3 — Normalize emails
    # Hint: df["email"] = df["email"].str.lower().str.strip()

    # TODO: Step 4 — Load into Silver
...
def transform_orders() -> pd.DataFrame:
    ...
    # if len(invalid) > 0:
    #     print(f"    ⚠️  {len(invalid)} orders with invalid status")

    # TODO: Step 3 — Convert order_date to datetime
    # Hint: df["order_date"] = pd.to_datetime(df["order_date"])

    # TODO: Step 4 — Replace NULL coupon_code with ''
    # Hint: df["coupon_code"] = df["coupon_code"].fillna("")
...
def transform_order_line_items() -> pd.DataFrame:
    ...
    # TODO: Step 2 — Validate quantity > 0
    # Hint: df = df[df["quantity"] > 0]

    # TODO: Step 3 — Check line_total calculation
    # Hint: df["_check"] = abs(df["line_total_usd"] - df["unit_price_usd"] * df["quantity"])
...
def transform_all() -> dict[str, pd.DataFrame]:
    ...
    # TODO: Call each transform_*() function and store the result
